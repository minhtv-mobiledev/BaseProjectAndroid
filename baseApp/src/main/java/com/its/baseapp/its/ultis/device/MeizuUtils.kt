...
import com.its.baseapp.its.ultis.permission.OverlayPermissionUtil
    ...
    fun applyPermission(context: Context) {
        try {
            ...
            (context as AppCompatActivity).startActivityForResult(intent,
                HuaweiUtils.REQUEST_CODE_FLOAT_WINDOW
            )
        } catch (e: Exception) {
            try {
                Log.e(TAG, "获取悬浮窗权限, 打开AppSecActivity失败, " + Log.getStackTraceString(e))
                // 最新的魅族flyme 6.2.5 用上述方法获取权限失败, 不过又可以用下述方法获取权限了
                OverlayPermissionUtil.commonROMPermissionApplyInternal(context)
            } catch (eFinal: Exception) {
                Log.e(
                    TAG,
                    "获取悬浮窗权限失败, 通用获取方法失败, " + Log.getStackTraceString(eFinal)
                )
            }
